...
public class HtlrTextField extends Gui implements Globals {
	...
	public HtlrTextField(int componentId, int x, int y, int par5Width, int par6Height) {
		this.id = componentId;
		this.x = x;
		this.y = y;
	...
	/**
	 * Called when mouse is clicked, regardless whether it is over this button
	 * or not.
	 */
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (this.isFocused && flag && mouseButton == 0) {
			...
			String s = FontUtils.trimStringToWidth(this.text.substring(this.lineScrollOffset), this.getWidth());
	...
	public void drawTextBox() {
		if (this.getVisible()) {
			...
			if (!s.isEmpty()) {
				...
				j1 = FontUtils.drawStringWithShadow(s1, (float) l, (float) i1, i);
			}
			...
			if (!s.isEmpty() && flag && j < s.length()) {
				j1 = FontUtils.drawStringWithShadow(s.substring(j), (float) j1, (float) i1, i);
			}

			if (flag1) {
				if (flag2) {
					Gui.drawRect(k1, i1 - 1, k1 + 1, i1 + 1 + FontUtils.getFontHeight(), -3092272);
				} else {
					FontUtils.drawStringWithShadow("_", (float) k1, (float) i1, i);
				}
			}

			if (k != j) {
				int l1 = l + FontUtils.getStringWidth(s.substring(0, k));
				this.drawSelectionBox(k1, i1 - 1, l1 - 1, i1 + 1 + FontUtils.getFontHeight());
			}
		}
	}
	...
	/**
	 * Sets whether the background and outline of this text box should be
	 * drawn.
	 */
	...
	public void setFocused(boolean isFocusedIn) {
		...
		if (mc.currentScreen != null) {
			mc.currentScreen.setFocused(isFocusedIn);
		}
	}
	...
	public void setSelectionPos(int position) {
		...
		int j = this.getWidth();
		String s = FontUtils.trimStringToWidth(this.text.substring(this.lineScrollOffset), j);
