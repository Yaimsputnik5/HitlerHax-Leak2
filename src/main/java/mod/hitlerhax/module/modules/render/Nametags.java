...
public class Nametags extends Module {
	...
	private void renderItemDurability(ItemStack itemStack, int posX, int posY) {
		...
		FontUtils.drawStringWithShadow((int) (damagePercent * 100) + "%", posX * 2, posY,
		...
		for (Enchantment enchantment : EnchantmentHelper.getEnchantments(itemStack).keySet()) {
			...
			if (protType.isEnabled()) {
				...
				if (enchantment.equals(Enchantments.BLAST_PROTECTION) || enchantment.equals(Enchantments.PROTECTION))
					FontUtils.drawStringWithShadow(ChatFormatting.BOLD + findStringForEnchants(enchantment, level),
							posX * 2 + 13, posY + 18, new ColorUtil(255, 255, 255, 255));
	...
	@EventHandler
	private final Listener<HtlrEventRenderEntityName> player_nametag = new Listener<>(event -> {
		event.cancel();
